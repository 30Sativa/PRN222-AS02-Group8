...
namespace OnlineLearningPlatform.Services.Implement
{
    public class PaymentService : IPaymentService
    {
        ...
        public string CreateVnPayTopUpUrl(Order order, HttpContext context)
        {
            var vnpay = new VnPayLibrary();
            ...
            vnpay.AddRequestData("vnp_IpAddr", VnPayLibrary.Utils.GetIpAddress(context));
            vnpay.AddRequestData("vnp_Locale", "vn");
            vnpay.AddRequestData("vnp_OrderInfo", $"Nap tien vao vi {order.OrderId}");
            vnpay.AddRequestData("vnp_OrderType", "topup");

            string returnUrl = _configuration["Vnpay:TopUpReturnUrl"] ?? "https://localhost:7088/Payment/VnPayTopUpCallback";
            vnpay.AddRequestData("vnp_ReturnUrl", returnUrl);
            vnpay.AddRequestData("vnp_TxnRef", order.OrderId.ToString());
        ...
        public async Task<(bool IsSuccess, int? OrderId, string Message)> ProcessTopUpCallbackAsync(IQueryCollection collections)
        {
            var vnpay = new VnPayLibrary();
            foreach (var (key, value) in collections)
            {
            ...
            if (int.TryParse(vnp_orderIdStr, out int orderId))
            {
                ...
                if (checkSignature)
                {
                    if (vnp_ResponseCode == "00")
                    {
                        bool success = await _orderService.CompleteTopUpOrderAsync(orderId, vnp_TransactionId, gatewayResponse);
                        return (success, orderId, success ? "Nạp tiền thành công" : "Lỗi cập nhật số dư ví");
                    }
                    else
                    {
                        return (false, orderId, "Nạp tiền bị hủy hoặc thất bại từ VNPAY.");
                    }
                }
                else
                {
